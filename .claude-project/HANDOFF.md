...
- ✅ Supabase 마이그레이션 2건(`migrate_add_exclude_from_budget.sql`, `migrate_add_budget_date_range.sql`) 사용자가 프로덕션에 적용 완료
...
1. 실 데이터로 예산 생성/수정, 시작일~종료일 자유 조정, 기간이 겹치는 예산 여러 개 동시 표시 여부를 종단 확인
2. 데일리 페이스 문구가 "예산 기간" 기준으로 정확히 계산되는지 확인 (특히 오늘 날짜가 기간 시작 전/종료 후인 경우, 기간이 1개월보다 짧거나 월 경계를 넘는 경우)
3. (선택) 오래된 로컬 백업 JSON을 가진 경우 가져오기(import) 시 `month` 필드만 있고 `start_date`/`end_date`가 없는 데이터를 어떻게 처리할지 결정 (Watch Out 참조)
4. (선택, 전전 세션 이월) Accounts 페이지 nav 진입 경로 추가, schema_version 갱신

## Blockers

없음 — 코드 작업 완료, 마이그레이션 2건(`migrate_add_exclude_from_budget.sql`, `migrate_add_budget_date_range.sql`) 모두 사용자가 프로덕션 Supabase에 적용 완료(2026-07-03). 실사용 종단 검증만 남음.
...
- `supabase/schema.sql` — 신규 설치 기준 스키마 반영
- `supabase/migrate_add_budget_date_range.sql` — 신규 마이그레이션 (프로덕션 적용 완료, 2026-07-03)
