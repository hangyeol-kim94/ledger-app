...
export default function AccountsPage() {
  ...
  // Compute balances reactively
  const balances = useLiveQuery(async () => {
    const all = await db.accounts.toArray()
    const entries = await Promise.all(
      all.map(async (a) => [a.id, await computeAccountBalance(a.id)] as const)
    )
  ...
  async function handleSave() {
    ...
    if (!name) {
      showToast('계좌 이름을 입력해주세요', 'error')
      return
    }
    const balanceNum = parseInt(form.initial_balance, 10)
    if (isNaN(balanceNum) || balanceNum < 0) {
      showToast('초기 잔액은 0 이상의 정수여야 합니다', 'error')
      return
    }
    ...
    try {
      if (form.id) {
        // Edit
        await db.accounts.update(form.id, {
          name,
          color: form.color,
          initial_balance: toAmount(balanceNum),
          memo: form.memo,
        })
  ...
  async function handleDeleteOrArchive(account: Account) {
    const txCount = await db.transactions.where('account_id').equals(account.id).count()
    const toCount = await db.transactions.where('to_account_id').equals(account.id).count()
    const total = txCount + toCount

    if (total > 0) {
      const ok = window.confirm(
        `${total}개 거래가 있습니다. 계좌를 보관하시겠습니까?\n(보관된 계좌는 거래 내역이 유지됩니다)`
      )
      if (!ok) return
  ...
  async function handleUnarchive(account: Account) {
    await db.accounts.update(account.id, { archived: false })
  ...
  async function moveAccount(account: Account, direction: -1 | 1) {
    ...
    await db.accounts.update(account.id, { sort_order: other.sort_order })
    await db.accounts.update(other.id, { sort_order: account.sort_order })
  }
    ...
    >
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          {account.memo && (
            <div style={{ fontSize: 12, opacity: 0.85, marginTop: 8 }}>{account.memo}</div>
          )}
        </div>
...
// Darken/lighten hex color by percentage
