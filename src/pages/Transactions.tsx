...
export default function TransactionsPage() {
  ...
  const list = useMemo<Transaction[]>(() => {
  ...
  }, [rawTransactions, filterAccountId, filterCategoryId, filterType])
  ...
  const grouped = useMemo(() => {
    ...
    return sortedDates.map((date) => ({
      date,
      items: groups.get(date)!.sort((a, b) => (a.created_at_utc < b.created_at_utc ? 1 : -1)),
    }))
  }, [list])
  ...
  async function handleDelete(id: string) {
    try {
      ...
      await updateTransaction(id, { deleted_at_utc: now, updated_at_utc: now })
  ...
  const hasActiveFilters = filterAccountId !== null || filterCategoryId !== null || filterType !== null

  const monthNavStyle: React.CSSProperties = { display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 16, padding: '12px 20px', background: '#FFFFFF', borderBottom: '1px solid #E2E8F0' }
  const arrowBtnStyle: React.CSSProperties = { background: 'transparent', color: '#1E293B', fontSize: 18, padding: '4px 10px', borderRadius: 8 }
  const filterBarStyle: React.CSSProperties = { display: 'flex', gap: 8, padding: '12px 20px', overflowX: 'auto', background: '#FFFFFF', borderBottom: '1px solid #E2E8F0' }
  const chipStyle = (active: boolean): React.CSSProperties => ({
    flexShrink: 0, padding: '6px 12px', borderRadius: 999, background: active ? '#EFF6FF' : '#F1F5F9',
    color: active ? '#2563EB' : '#1E293B', border: active ? '1px solid #2563EB' : '1px solid transparent',
    fontSize: 12, fontWeight: 500, whiteSpace: 'nowrap',
  })
  const summaryStyle: React.CSSProperties = { display: 'flex', gap: 16, alignItems: 'center', padding: '12px 20px', background: '#F8FAFC', fontSize: 13, color: '#1E293B', borderBottom: '1px solid #E2E8F0' }
  const dateHeaderStyle: React.CSSProperties = { padding: '14px 20px 6px', fontSize: 13, fontWeight: 600, color: '#94A3B8', background: '#F8FAFC' }
  ...
  return (
    <div className="page">
      <div className="page-header">
        <h1>거래내역</h1>
        {hasActiveFilters && (
          <button onClick={resetFilters} style={{ background: 'transparent', color: '#2563EB', fontSize: 13, fontWeight: 600, padding: '4px 8px' }}>
            필터 초기화
          </button>
        )}
      </div>
      ...
      <div style={monthNavStyle}>
        <button style={arrowBtnStyle} onClick={() => setSelectedMonth(navigateMonth(selectedMonth, -1))} aria-label="이전 달">‹</button>
        <div style={{ fontSize: 15, fontWeight: 700, color: '#1E293B', minWidth: 100, textAlign: 'center' }}>{formatMonthKorean(selectedMonth)}</div>
        <button style={arrowBtnStyle} onClick={() => setSelectedMonth(navigateMonth(selectedMonth, 1))} aria-label="다음 달">›</button>
      </div>

      <div style={filterBarStyle} className="scroll-x">
        <button style={chipStyle(filterType === null)} onClick={() => setFilterType(null)}>전체</button>
        {(['income', 'expense', 'transfer'] as TransactionType[]).map((t) => (
          <button
            key={t}
            style={{ ...chipStyle(filterType === t), color: filterType === t ? TYPE_COLOR[t] : '#1E293B', borderColor: filterType === t ? TYPE_COLOR[t] : 'transparent', background: filterType === t ? TYPE_COLOR[t] + '14' : '#F1F5F9' }}
            onClick={() => setFilterType(t)}
          >
            {TYPE_LABEL[t]}
          </button>
        ))}
        <select value={filterAccountId ?? ''} onChange={(e) => setFilterAccountId(e.target.value || null)} style={{ ...chipStyle(filterAccountId !== null), border: filterAccountId !== null ? '1px solid #2563EB' : '1px solid transparent', outline: 'none', cursor: 'pointer' }}>
      ...
      <div style={summaryStyle}>
        ...
        <span style={{ color: '#10B981', fontWeight: 600 }}>수입 +{formatKRW(summary.income)}</span>
        <span style={{ color: '#EF4444', fontWeight: 600 }}>지출 -{formatKRW(summary.expense)}</span>
      </div>

      {grouped.length === 0 ? (
        <div style={{ padding: '60px 20px', textAlign: 'center', color: '#94A3B8', fontSize: 14 }}>거래내역이 없습니다</div>
      ) : (
        grouped.map((group) => (
          <div key={group.date}>
